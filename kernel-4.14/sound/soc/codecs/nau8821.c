	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	struct nau8821 *nau8821 = snd_soc_codec_get_drvdata(codec);
...
static int nau8821_dac_event(struct snd_soc_dapm_widget *w,
	struct snd_kcontrol *kcontrol, int event)
{
	struct snd_soc_codec *codec = snd_soc_dapm_to_codec(w->dapm);
	struct nau8821 *nau8821 = snd_soc_codec_get_drvdata(codec);
	...
	if (SND_SOC_DAPM_EVENT_ON(event)) {
		regmap_update_bits(nau8821->regmap,
			NAU8821_REG_HSVOL_CTRL,
			NAU8821_HP_MUTE, NAU8821_HP_MUTE);
		msleep(30);
	}
	return 0;
}

static int nau8821_classg_event(struct snd_soc_dapm_widget *w,
	struct snd_kcontrol *kcontrol, int event)
{
	struct snd_soc_codec *codec = snd_soc_dapm_to_codec(w->dapm);
	struct nau8821 *nau8821 = snd_soc_codec_get_drvdata(codec);
	...
	if (SND_SOC_DAPM_EVENT_ON(event)) {
		regmap_update_bits(nau8821->regmap,
			...
			NAU8821_HP_MUTE, 0);
		regmap_update_bits(nau8821->regmap,
			NAU8821_REG_MUTE_CTRL,
			NAU8821_DAC_SOFT_MUTE, 0);
		msleep(30);
	} else if (SND_SOC_DAPM_EVENT_OFF(event)) {
		regmap_update_bits(nau8821->regmap,
			NAU8821_REG_MUTE_CTRL,
			NAU8821_DAC_SOFT_MUTE, NAU8821_DAC_SOFT_MUTE);
		regmap_update_bits(nau8821->regmap,
			NAU8821_REG_HSVOL_CTRL,
			NAU8821_HP_MUTE, NAU8821_HP_MUTE);
		msleep(30);
...
static const struct snd_soc_dapm_widget nau8821_dapm_widgets[] = {
	...
	SND_SOC_DAPM_DAC_E("DDACR", NULL, NAU8821_REG_ENA_CTRL,
		NAU8821_EN_DACR_SFT, 0,
		nau8821_dac_event, SND_SOC_DAPM_PRE_PMU),
	SND_SOC_DAPM_DAC_E("DDACL", NULL, NAU8821_REG_ENA_CTRL,
	...
	SND_SOC_DAPM_PGA_S("Class G", 10,
		NAU8821_REG_CLASSG_CTRL, NAU8821_CLASSG_EN_SFT, 0,
		nau8821_classg_event, SND_SOC_DAPM_POST_PMU |
...
int nau8821_startup(struct snd_pcm_substream *substream,
...
static int nau8821_codec_probe(struct snd_soc_codec *codec)
{
	struct nau8821 *nau8821 = snd_soc_codec_get_drvdata(codec);
...
static int nau8821_codec_remove(struct snd_soc_codec *codec)
{
	struct nau8821 *nau8821 = snd_soc_codec_get_drvdata(codec);
 ...
 */
static int nau8821_set_fll(struct snd_soc_codec *codec, int pll_id, int source,
	unsigned int freq_in, unsigned int freq_out)
{
	...
	if (ret) {
		dev_err(codec->dev, "Unsupported input clock %d to output clock %d\n",
			freq_in, freq_out);
		return ret;
	}
...
static int nau8821_set_sysclk(struct snd_soc_codec *codec, int clk_id,
...
static int nau8821_set_bias_level(struct snd_soc_codec *codec,
...
static int __maybe_unused nau8821_suspend(struct snd_soc_codec *codec)
{
	struct nau8821 *nau8821 = snd_soc_codec_get_drvdata(codec);
...
static int __maybe_unused nau8821_resume(struct snd_soc_codec *codec)
{
	struct nau8821 *nau8821 = snd_soc_codec_get_drvdata(codec);
...
static const struct snd_soc_codec_driver nau8821_codec_driver = {
	.probe = nau8821_codec_probe,
 ...
 * reporting.
 */
int nau8821_enable_jack_detect(struct snd_soc_codec *codec,
	...
	const struct i2c_device_id *id)
{
	...
	return snd_soc_register_codec(&i2c->dev, &nau8821_codec_driver,
		&nau8821_dai, 1);
}
